...
def train(train_dataloader, test_dataloader, model, args):
    ...
    for e in range(args.epoch):
        ...
        for _, (g_cpu, edge_ratio, edge_flow) in enumerate(train_dataloader):
            g = g_cpu.to(device)
        ...
        if e % 5 == 0:
            ...
            model.eval()
            for _, (g_cpu, edge_ratio, edge_flow) in enumerate(test_dataloader):
                g = g_cpu.to(device)
                edge_ratio = edge_ratio.to(device)
                edge_flow = edge_flow.to(device)
            ...
            if np.mean(np.array(test_loss)) < min_test_loss:
                ...
                torch.save(model.state_dict(), 'model/{}_{}_{}_{}_{}_{}.pth'.format(
                    ' '.join(args.train_data_dir_list), args.model_idx, args.train_ratio, args.test_ratio, args.conservation_loss, args.loss))
    ...
    with open('./saved_result/{}_{}_{}_{}_{}_{}.pickle'.format(
        ' '.join(args.train_data_dir_list), args.model_idx, args.train_ratio, args.test_ratio, args.conservation_loss, args.loss), 'wb') as handle:
